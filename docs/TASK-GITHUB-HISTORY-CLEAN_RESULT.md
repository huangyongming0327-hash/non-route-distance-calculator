...
- 原污染标签：均已由 `git-filter-repo` 重写到安全历史；`v1.0` 已在正式包验证后重新创建并指向清理后的正式构建提交。
...
- 清理后的 `master`：`9f8f293e523c72d2d4ec4b97e5da9a2227da2100`。
- 清理后的 `release/v1.0` 正式构建提交：`4075063ff8c0425715bbedbf70b6ffc10dd4ca9e`；本报告及最终证据在其后以独立文档提交保存。
- 清理后的 `v1.0` 标签目标：`4075063ff8c0425715bbedbf70b6ffc10dd4ca9e`。
- 标签说明：`非线路运距计算工具V1.0正式版（GitHub首次发布安全清理版）`。
- 新 EXE SHA-256：`F5ADE01A2415AECA7CA2FF60D9F7063EB3FC9794686C7DA65F7CAF1C9AEC8A91`；3,695,562 字节。
- 新 ZIP SHA-256：`8A33F080F6BF040B122EE6C35335577E3BB0F8D6B9EFE30F58F664BF159F9ED1`；55,208,960 字节。
- 正式目录、中文路径和带空格路径启动验证：3/3 通过，主窗口最慢 1.563 秒出现。
- 冻结 EXE Excel/WPS worker：2/2 通过，VBA 保留，宏和事件禁用，Office 进程隔离通过。
- 正式目录和 ZIP 安全扫描：各 191 个文件，0 命中；五个运行目录均为空。
- 包内 `VERSION.txt`：记录正式构建提交 `4075063ff8c0425715bbedbf70b6ffc10dd4ca9e` 和 `clean` 构建状态。
- 首次 GitHub 推送条件：满足；仍须按后续首次推送任务执行认证、添加远程和推送，本任务未执行这些操作。
